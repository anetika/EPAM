...
public class GoToAllIrrelevantVacanciesPageCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        ...
        try {
            List<Vacancy> vacancies = service.findVacanciesByStatus(Vacancy.VacancyStatus.IRRELEVANT);
            request.setAttribute(RequestAttribute.VACANCIES_LIST, vacancies);
            router = new Router(PagePath.ALL_IRRELEVANT_VACANCIES_PAGE, Router.RouterType.FORWARD);
        } catch (ServiceException e) {
            logger.error("Internal server error in GoToAllIrrelevantVacanciesPageCommand", e);
